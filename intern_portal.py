...
# ==========================================
# 1. 初期設定 & データベース初期化 (PostgreSQL / Neon対応)
# ==========================================
...
# 接続用ライブラリのインポート（Neon接続用）
try:
    import psycopg2
    from psycopg2.extras import RealDictCursor
except ImportError:
    st.error("⚠️ `psycopg2-binary` がインストールされていません。requirements.txtに追記してください。")
...
def get_db_connection():
    # Neon(PostgreSQL)に接続。未設定ならローカル（予備）
    if db_url:
        # 💡 neon側の要求に合わせてURLを少し調整
...
# 3. Streamlit UI 構築
# ==========================================
active_tab = st.radio("メニュー", ["📋 候補者一覧・登録", "🔎 面接・評価（詳細画面）"], horizontal=True, label_visibility="collapsed")
...
if active_tab == "📋 候補者一覧・登録":
    ...
    with st.expander("➕ 新しい候補者を手動で追加する"):
        with st.form("add_candidate_form"):
            ...
            if st.form_submit_button("候補者を登録") and c_name:
        ...
        for index, row in df.iterrows():
            ...
            with c_col5:
                if st.button("詳細・面接へ 🔍", key=f"det_{row['id']}"):
                    ...
                    st.success(f"🎯 【{row['name']}】さんの選考データをセットしました！上の「🔎 面接・評価」を選んでください！")
    else:
...
if active_tab == "🔎 面接・評価（詳細画面）":
    ...
    if not candidates_list:
        st.info("候補者が登録されていません。一覧画面から詳細ボタンを押してください。")
    else:
        ...
        with sub_tabs[0]:
            ...
            if st.button("AI相性診断＆事前アドバイスを生成（自動保存）", key="pre_ai_btn"):
                with st.spinner("分析中..."):
                    ...
                    prompt = f"マキコ（数秘8, {my_fortune_str}）と候補者{name}（数秘{c_num}, {c_fortune_str}）の相性を分析し、300字以内で1.特徴 2.姿勢 3.攻略法 4.フランクな質問3選 を出力して。"
                    ai_res = ask_gemini(prompt)
                    saved_pre_profile = ai_res
                    
                    new_memo_content = raw_memo + f"\n\n【AI事前カンペ】\n{ai_res}" if "【AI事前カンペ】" not in raw_memo else raw_memo
        ...
        with sub_tabs[1]:
            ...
            updated_memo = st.text_area("面接メモ", value=display_memo, height=500, key=f"memo_{c_id}")
            ...
            if st.button("確認漏れ・追加質問をAIに聞く"):
                with st.spinner("分析中..."):
                    prompt = f"以下メモから不足する項目を2つ選び、フランクな質問のセリフと150字の背景を出力して。\n{updated_memo}"
                    st.warning(ask_gemini(prompt))

        with sub_tabs[2]:
            if st.button("AIレポートを生成する"):
                with st.spinner("生成中..."):
                    ai_report = ask_gemini(f"以下メモから8項目について強みと懸念、活躍可能性をまとめて。\n{updated_memo}")
                    conn = get_db_connection()
                    cursor = conn.cursor()
